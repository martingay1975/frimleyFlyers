define(['raceEventResultEntry', 'competitorsData', 'jquery-dateFormat', 'raceDistance'], function (RaceEventResultEntry, competitorsData, dateFormat, raceDistance) {
	...
	var RaceEvent = function(seasonYear, date, distance) {
		...
		this.addCompetitorResult = function(name, time) {
			self.results.push(new RaceEventResultEntry(self.seasonYear, name, time, self.distance));
		};
	};

	var Race = function(seasonYear, pointsScheme, pointsPbBonus, label, distance, website, image) {
		...
		this.findInPositions = function(name) {
			var raceEventResultEntry = RaceEventResultEntry.findFromName(self.positions, name);
			return raceEventResultEntry;
		}

		this.generatePositions = function() {
			var competitorBestEvent, orderedBestEvents = [];

			// loop around each person
			competitorsData.getCompetitors(seasonYear).forEach(function(person) {
		...
		this.assignPoints = function() {
			...
			for (index = 0; index < self.positions.length; index += 1) {
				if (index < self.pointsScheme.length) {
					...
					if (checkPreviousIndex > -1 &&
						self.positions[index].percentageFromPb === self.positions[checkPreviousIndex].percentageFromPb) {
						// there are equal %s from pb - so share the points.
						var averagePoints = (self.pointsScheme[index] + self.pointsScheme[checkPreviousIndex]) / 2;
						self.positions[index].points = averagePoints;
						self.positions[checkPreviousIndex].points = averagePoints;

						if (self.positions[checkPreviousIndex].isBetterThanPb) {
							self.positions[checkPreviousIndex].points += self.pointsPbBonus;
						}
					}
				}
		...
		placeInResults = function(array, raceEventResultEntry) {
			...
			for (index = 0; index < array.length; index += 1) {
				...
				if (raceEventResultEntry.percentageFromPb < currentRaceEventResultEntry.percentageFromPb) {
					indexAtInsert = index;
					break;
				}
			}
			...
			self.raceEvents.forEach(function (raceEvent) {
				...
				if (raceEventResultEntry) {
					if (bestEvent) {
						...
						if (raceEventResultEntry.percentageFromPb < bestEvent.percentageFromPb) {
