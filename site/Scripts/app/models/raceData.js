// season data
define(["jquery", "timeSpan", "race", "competitorsData", "siteOptions"], function ($, TimeSpan, Race, competitorsData, siteOptions) {
	...
	var RaceData = function (seasonYear) {
		...
		addRacesAsync = function () {
			...
			getRaceDataPromise.fail(function(err, err2, err3, err4) {
				debugger;
				window.alert("getRaceDataPromise failed. " + err3 + " " + url + ". Try using err.responseText.substring(startPos, endPos)");
			});

			var completedPromise = getRaceDataPromise.then(function (racesDataJson) {
				...
				self.leagueTableColumnsToShow = racesDataJson.leagueTableColumnsToShow || [];
				var personCounter = 1;
				racesDataJson.records.forEach(function(recordJson) {
					
					var person = competitorsData.getCompetitor(recordJson.name);

					if (!person) {
						throw "Unable to find person '" + recordJson.name + "' in '" + url + "'";
					}

					var fiveKm = new TimeSpan(recordJson.fiveKm.hour, recordJson.fiveKm.minute, recordJson.fiveKm.second);

					var tenKm = null;
					if (recordJson.tenKm) {
						tenKm = new TimeSpan(recordJson.tenKm.hour, recordJson.tenKm.minute, recordJson.tenKm.second);
					}

					var halfMarathon = null;
					if (recordJson.halfMarathon) {
						halfMarathon = new TimeSpan(recordJson.halfMarathon.hour,recordJson.halfMarathon.minute,recordJson.halfMarathon.second);
					}
				...
				racesDataJson.races.forEach(function (raceDataJson) {
					var race = new Race(seasonYear, self.pointsScheme, self.pointsPbBonus, raceDataJson.label, raceDataJson.distance, raceDataJson.website, raceDataJson.icon);

					// Add each of the events that constitute the race
					raceDataJson.events.forEach(function (raceEventsJson) {
						var event = race.addRaceEvent(new Date(raceEventsJson.date[0], raceEventsJson.date[1], raceEventsJson.date[2]),
		...
		init = function () {
			...
			var addRaceRacesProcessedPromise = addRacesPromise.then(function() {
				try
				{
					calculateRaceResults.call(self);
					competitorsData.calculateCompetitorsTotalPoints(self.races, self.takeNBestScores);
